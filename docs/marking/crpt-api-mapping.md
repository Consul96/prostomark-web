...
**Браузерный smoke (live).** Поднят реальный стек (uvicorn + vite с dev-прокси
`/api` → backend), вход выполнен через UI. Проверено вживую: пункт меню «Честный
знак» встроен в существующее приложение (тот же shell/бренд ProstoMark); dashboard
с 10 KPI-карточками и пустым состоянием «Нет требующих внимания действий»; список
клиентов (пустое состояние → создание клиента «ООО Ромашка» через форму); статусы
после создания — честные «Не задана / Не проверено» (не поддельный CONNECTED);
кнопка «Проверить» переводит True API/СУЗ в «Требуется настройка» (needs_setup),
т.к. агент/omsId не заданы; вкладка «История» показывает аудит `client.create /
7701234567 / ok`; адаптив на 375px — сайдбар и суб-навигация переносятся без
горизонтального скролла. Визуально модуль совпадает с дизайн-системой приложения
(бренд-палитра, Manrope, карточки `rounded-2xl`).

> Figma-макет `figma.com/design/QcqY0AwteIVCfXS31j92wR` в текущем окружении открыть
> нельзя (Figma-коннектор требует OAuth, недоступного в неинтерактивной сессии).
> UI выровнен по уже утверждённой дизайн-системе `prostomark-web` (те же компоненты
> и токены, что и на остальных экранах приложения). Точная сверка с Figma — при
> наличии доступа.

**Найденный предсуществующий баг базового приложения (вне scope marking).**
Регистрация и вход (`/api/v1/auth/*`) не работают против БД, поднятой Alembic-ом:
Postgres-enum `user_role_enum` создан миграцией со значениями в нижнем регистре
(`company_admin`), а SQLAlchemy-колонка `User.role` сохраняет ИМЯ enum-члена
(`COMPANY_ADMIN`) → `InvalidTextRepresentation`. Тесты этого не ловят, т.к.
`conftest` строит схему через `create_all` (enum совпадает с моделью), а не через
Alembic. Модуль marking не затронут — статусы хранятся как обычные строковые
колонки. Заведена отдельная задача на исправление (values_callable + регресс-тест
против Alembic-схемы).
